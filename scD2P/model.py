...
def fit_scD2P(
    ...
    if not np.any(is_control):
        raise ValueError(f"No control cells found with {perturbation_col} == {ctrl_label!r}.")
    if np.all(is_control):
        raise ValueError("At least one perturbed cell is required to fit perturbation programs.")
    ...
    if not np.any(gene_mask):
        raise ValueError(
            "No expressed genes were found in control cells after non-negative filtering."
        )
